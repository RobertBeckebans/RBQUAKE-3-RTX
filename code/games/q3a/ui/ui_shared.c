			...
			for( i = listPtr->startPos; i < count; i++ )
			{
				...
				if( listPtr->numColumns > 0 )
				{
					...
					for( j = 0; j < listPtr->numColumns; j++ )
					{
						...
						{
							DC->drawText(
								x + 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight, item->textscale, item->window.foreColor, text, 0, listPtr->columnInfo[j].maxChars, item->textStyle );
						}
					}
				}
				else
...
void Item_ValidateTypeData( itemDef_t* item )
{
	...
	else if( item->type == ITEM_TYPE_EDITFIELD || item->type == ITEM_TYPE_NUMERICFIELD || item->type == ITEM_TYPE_YESNO || item->type == ITEM_TYPE_BIND || item->type == ITEM_TYPE_SLIDER ||
