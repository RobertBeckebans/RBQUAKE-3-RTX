...
static const char* master_items[] = { "Local", "Internet", "Favorites", 0 };

static const char* servertype_items[] = { "All", "Free For All", "Team Deathmatch", "Tournament", "Capture the Flag", 0 };

static const char* sortkey_items[] = { "Server Name", "Map Name", "Open Player Spots", "Game Type", "Ping Time", 0 };
...
static void ArenaServers_UpdateMenu()
{
	...
	for( i = 0, j = 0; i < count; i++, servernodeptr++ )
	{
		...
		Com_sprintf( buff,
			MAX_LISTBOXWIDTH,
			"%-20.20s %-12.12s %2d/%2d %-8.8s %3s %s%3d " S_COLOR_YELLOW "%s",
...
=================
*/
void ArenaServers_InsertFavorites()
{
	int	 i;
	int	 j;
