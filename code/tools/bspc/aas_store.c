...
qboolean AAS_GetVertex( vec3_t v, int* vnum )
{
	...
	{
		if( fabs( aasworld.vertexes[vn][0] - vert[0] ) < VERTEX_EPSILON && fabs( aasworld.vertexes[vn][1] - vert[1] ) < VERTEX_EPSILON && fabs( aasworld.vertexes[vn][2] - vert[2] ) < VERTEX_EPSILON )
		{
			*vnum = vn;
			return true;
		} // end if
