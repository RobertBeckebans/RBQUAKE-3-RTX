		...
		for( instruction = 0; instruction < header->instructionCount; instruction++ )
		{
			...
			switch( op )
			{
				...
				case OP_STORE2:
					...
					assertInteger( opStackDepth - 1 );
					assertInteger( opStackDepth - 2 );
					Inst( "sthx", PPC_STHX, opStackIntRegisters[ opStackDepth - 1 ], opStackIntRegisters[ opStackDepth - 2 ], R_MEMBASE ); // store from memory base
					opStackRegType[ opStackDepth - 1 ]             = 0;
					opStackRegType[ opStackDepth - 2 ]             = 0;
					opStackLoadInstructionAddr[ opStackDepth - 1 ] = 0;
					opStackLoadInstructionAddr[ opStackDepth - 2 ] = 0;
				...
				case OP_STORE1:
					...
					assertInteger( opStackDepth - 1 );
					assertInteger( opStackDepth - 2 );
					Inst( "stbx", PPC_STBX, opStackIntRegisters[ opStackDepth - 1 ], opStackIntRegisters[ opStackDepth - 2 ], R_MEMBASE ); // store from memory base
					opStackRegType[ opStackDepth - 1 ]             = 0;
					opStackRegType[ opStackDepth - 2 ]             = 0;
					opStackLoadInstructionAddr[ opStackDepth - 1 ] = 0;
					opStackLoadInstructionAddr[ opStackDepth - 2 ] = 0;
...
int VM_CallCompiled( vm_t* vm, int* args )
{
	...
	*( int* )&image[ programStack ]      = -1; // will terminate the loop on return
	...
	( ( void ( * )( int, int, int, int, int, int, int, int ) )( vm->codeBase ) )(
		programStack, ( int )&stack, ( int )image, vm->dataMask, ( int )&AsmCall, ( int )vm->instructionPointers, vm->instructionPointersLength, ( int )vm );
#else
	( ( void ( * )( int, int, int, int, int, int, int, int ) )( vm->codeBase ) )(
		programStack, ( int )&stack, ( int )image, vm->dataMask, *( int* )&AsmCall /* skip function pointer header */, ( int )vm->instructionPointers, vm->instructionPointersLength, ( int )vm );
#endif
