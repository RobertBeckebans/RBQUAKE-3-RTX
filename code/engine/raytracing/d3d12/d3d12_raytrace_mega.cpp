...
void GL_LoadMegaTexture( D3D12_CPU_DESCRIPTOR_HANDLE& srvPtr )
{
	...
	// Create the texture.
	{
		...
		ThrowIfFailed( m_device->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES( D3D12_HEAP_TYPE_DEFAULT ), D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS( &megaTexture.texture2D ) ) );
...
void GL_LoadMegaNormalTexture( D3D12_CPU_DESCRIPTOR_HANDLE& srvPtr )
{
	...
	// Create the texture.
	{
		...
		ThrowIfFailed( m_device->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES( D3D12_HEAP_TYPE_DEFAULT ), D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS( &megaTextureNormal.texture2D ) ) );
