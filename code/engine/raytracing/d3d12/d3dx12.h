...
struct CD3DX12_ROOT_DESCRIPTOR_TABLE : public D3D12_ROOT_DESCRIPTOR_TABLE
{
	...
	inline void Init( UINT numDescriptorRanges, _In_reads_opt_( numDescriptorRanges ) const D3D12_DESCRIPTOR_RANGE* _pDescriptorRanges ) { Init( *this, numDescriptorRanges, _pDescriptorRanges ); }
...
struct CD3DX12_ROOT_DESCRIPTOR_TABLE1 : public D3D12_ROOT_DESCRIPTOR_TABLE1
{
	...
	inline void Init( UINT numDescriptorRanges, _In_reads_opt_( numDescriptorRanges ) const D3D12_DESCRIPTOR_RANGE1* _pDescriptorRanges ) { Init( *this, numDescriptorRanges, _pDescriptorRanges ); }

	static inline void Init(
		_Out_ D3D12_ROOT_DESCRIPTOR_TABLE1& rootDescriptorTable, UINT numDescriptorRanges, _In_reads_opt_( numDescriptorRanges ) const D3D12_DESCRIPTOR_RANGE1* _pDescriptorRanges )
	{
		rootDescriptorTable.NumDescriptorRanges = numDescriptorRanges;
		rootDescriptorTable.pDescriptorRanges	= _pDescriptorRanges;
	}
...
struct CD3DX12_ROOT_DESCRIPTOR1 : public D3D12_ROOT_DESCRIPTOR1
{
	...
	CD3DX12_ROOT_DESCRIPTOR1( UINT shaderRegister, UINT registerSpace = 0, D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE ) { Init( shaderRegister, registerSpace, flags ); }

	inline void		   Init( UINT shaderRegister, UINT registerSpace = 0, D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE ) { Init( *this, shaderRegister, registerSpace, flags ); }
...
struct CD3DX12_CPU_DESCRIPTOR_HANDLE : public D3D12_CPU_DESCRIPTOR_HANDLE
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE( _In_ const D3D12_CPU_DESCRIPTOR_HANDLE& other, INT offsetScaledByIncrementSize ) { InitOffsetted( other, offsetScaledByIncrementSize ); }
...
struct CD3DX12_GPU_DESCRIPTOR_HANDLE : public D3D12_GPU_DESCRIPTOR_HANDLE
{
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE( _In_ const D3D12_GPU_DESCRIPTOR_HANDLE& other, INT offsetScaledByIncrementSize ) { InitOffsetted( other, offsetScaledByIncrementSize ); }
	...
	inline void InitOffsetted( _In_ const D3D12_GPU_DESCRIPTOR_HANDLE& base, INT offsetScaledByIncrementSize ) { InitOffsetted( *this, base, offsetScaledByIncrementSize ); }
...
{
	if( ppErrorBlob != NULL ) { *ppErrorBlob = NULL; }
		...
		case D3D_ROOT_SIGNATURE_VERSION_1_0:
			switch( pRootSignatureDesc->Version )
			{
				...
				case D3D_ROOT_SIGNATURE_VERSION_1_1:
				{
					...
					if( ParametersSize > 0 && pParameters == NULL ) { hr = E_OUTOFMEMORY; }
						...
						for( UINT n = 0; n < desc_1_1.NumParameters; n++ )
						{
							...
							switch( desc_1_1.pParameters[n].ParameterType )
							{
								...
								case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE:
									...
									if( DescriptorRangesSize > 0 && pDescriptorRanges == NULL ) { hr = E_OUTOFMEMORY; }
...
struct CD3DX12_PIPELINE_STATE_STREAM1
{
	...
	CD3DX12_PIPELINE_STATE_STREAM1( const D3D12_GRAPHICS_PIPELINE_STATE_DESC& Desc ) :
		Flags( Desc.Flags ), NodeMask( Desc.NodeMask ), pRootSignature( Desc.pRootSignature ), InputLayout( Desc.InputLayout ), IBStripCutValue( Desc.IBStripCutValue ),
		PrimitiveTopologyType( Desc.PrimitiveTopologyType ), VS( Desc.VS ), GS( Desc.GS ), StreamOutput( Desc.StreamOutput ), HS( Desc.HS ), DS( Desc.DS ), PS( Desc.PS ),
		BlendState( CD3DX12_BLEND_DESC( Desc.BlendState ) ), DepthStencilState( CD3DX12_DEPTH_STENCIL_DESC1( Desc.DepthStencilState ) ), DSVFormat( Desc.DSVFormat ),
		RasterizerState( CD3DX12_RASTERIZER_DESC( Desc.RasterizerState ) ), RTVFormats( CD3DX12_RT_FORMAT_ARRAY( Desc.RTVFormats, Desc.NumRenderTargets ) ), SampleDesc( Desc.SampleDesc ),
