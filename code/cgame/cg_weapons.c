...
static void CG_RocketTrail( centity_t* ent, const weaponInfo_t* wi )
{
	...
	for( ; t <= ent->trailTime; t += step )
	{
		...
		smoke = CG_SmokePuffModel( lastPos, up, wi->trailRadius, 1, 1, 1, 0.33f, wi->wiTrailTime, t, 0, 0, cgs.media.smokePuffModel );
		// use the optimized local entity add
		smoke->leType = LE_SCALE_FADE;
	}
}
...
void CG_AddPlayerWeapon( refEntity_t* parent, playerState_t* ps, centity_t* cent, int team )
{
		...
		cent->currentState.number != cg.predictedPlayerState.clientNum )
	{
		...
		{
			trap_R_AddLightToScene( flash.origin, 300 + ( rand() & 31 ), weapon->flashDlightColor[ 0 ], weapon->flashDlightColor[ 1 ], weapon->flashDlightColor[ 2 ], LDAT_QUADRAT );
		}
	}
}
